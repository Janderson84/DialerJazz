...
export default function ManualDialerPage() {
  ...
  if (voice.error) {
    const isConfigGap = /connectors|credentials|not connected/i.test(voice.error);
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center max-w-md mx-auto">
        <AlertCircle className="h-12 w-12 text-amber-500 mb-4" />
        <h2 className="text-2xl font-bold text-foreground mb-2">{isConfigGap ? 'Telephony setup needed' : 'Error Loading Dialer'}</h2>
        <p className="text-muted-foreground mb-6">{voice.error}</p>
        {isConfigGap && (
          <button onClick={() => navigate('/connectors')} className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-2 rounded-xl transition-colors font-medium mb-3">Open Connectors</button>
        )}
        <button onClick={() => navigate('/login')} className="bg-muted hover:bg-muted/80 text-foreground px-6 py-2 rounded-xl transition-colors hover:bg-white/20 font-medium">{isConfigGap ? 'Re-authenticate anyway' : 'Re-authenticate'}</button>
      </div>
    );
  }
