...
// Master-only nav: reads signed-in user id from the stored JWT (no AuthContext
// import here — importing it created a module cycle that blanked the whole app).
function isTeamMaster(): boolean {
  try {
    const token = localStorage.getItem('jazz_access_token');
    if (!token) return false;
    const payload = JSON.parse(atob(token.split('.')[1] || ''));
    return payload?.sub === 'a4d41720-59e1-4850-8b15-e8841872e702';
  } catch {
    return false;
  }
}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-1">
                  ...
                  {isTeamMaster() && <NavItem href="/team" icon={UserCog} active={pathname?.includes("team")} onNavigate={handleNavigation}>Team</NavItem>}
                </div>
              </div>
            </div>
          </div>
